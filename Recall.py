import pandas as pd  #pandas 用来处理表格数据 .csv这种
import numpy as np  #主要用来进行矩阵运算之类的（数值运算）
from tqdm import tqdm  #进度条 可以让你知道运行进度
from collections import defaultdict  #缺省字典 用来避免报错的（单纯的一个空字典 再添加东西时如果没有那个元素会报错 但是defaultdict 会自己创建一个）
import os, math, warnings, pickle  #os 用来判断文件路径是否正确 创建文件等  pickle
import faiss #facebook 开源的向量检索库 用来做ANN 近似最近邻向量检索
import collections
import random
import tensorflow as tf
from sklearn.preprocessing import MinMaxScaler  #归一化工具
from sklearn.preprocessing import LabelEncoder  # 将离散类别转变为数字
from datetime import datetime
from deepctr.feature_column import SparseFeat, VarLenSparseFeat #SparseFeat 用来把离散特征转换为embedding向量 VarLenSparseFeat用来把 变长离散特征转换为embedding向量
...
# 与 TensorFlow 2.20 / Keras 3 不兼容。在项目内补丁，避免修改系统包。
def _compatible_get_item_embedding(item_embedding, item_input_layer):
    # DeepMatch only stores this result as the optional ``model.item_embedding``
    # helper.  Recall below reads the trained embedding layer weights directly,
    # so a placeholder avoids TF/Keras symbolic-tensor incompatibilities without
    # changing the YoutubeDNN training graph or the embeddings used for recall.
    del item_embedding
    return item_input_layer


YoutubeDNN.__globals__['get_item_embedding'] = _compatible_get_item_embedding
...
def get_all_click_sample(data_path, sample_nums=10000):
    ...
    """
    all_click = pd.read_csv(Path(data_path) / 'train_click_log.csv')
...
# 全量训练集
all_click_df = get_all_click_df(data_path, offline=OFFLINE)
...
def youtubednn_u2i_dict(data, topk=20):    
    data = data.copy()  # LabelEncoder 会原地修改列，避免污染后续召回使用的原始 ID
    raw_user_history = data.groupby('user_id')['click_article_id'].agg(set).to_dict()
    ...
    # click_article_id 与 hist_article_id 共享该 Embedding 层；直接读取训练后的权重，
    # 可绕过旧版 DeepMatch 在 Keras 3 下失效的 item 子模型。
    item_embs = model.get_layer('sparse_seq_emb_hist_article_id').get_weights()[0]
...
#     faiss.normalize_L2(item_embs)
    ...
    for target_idx, sim_value_list, rele_idx_list in tqdm(zip(test_user_model_input['user_id'], sim, idx)):
        ...
        # 用户向量检索不是 item-to-item 检索，不能无条件丢弃第一名；只过滤已点击文章。
        for rele_idx, sim_value in zip(rele_idx_list, sim_value_list): 
            rele_raw_id = item_index_2_rawid[rele_idx]
            if rele_raw_id in raw_user_history.get(target_raw_id, set()):
                continue
...
def item_based_recommend(user_id, user_item_time_dict, i2i_sim, sim_item_topk, recall_item_num, item_topk_click, item_created_time_dict, emb_i2i_sim):
    ...
    if len(item_rank) < recall_item_num:
        for i, item in enumerate(item_topk_click):
            if item in item_rank or item in user_hist_items_: # 填充文章不能已出现或已被用户点击
                continue
...
if metric_recall:
    trn_hist_click_df,_ = get_hist_and_last_click(
        all_click_df
    )
else:
    trn_hist_click_df = all_click_df
