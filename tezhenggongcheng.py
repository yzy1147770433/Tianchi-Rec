...
def trn_val_split(all_click_df, sample_user_nums):
    ...
    sample_nums = min(sample_user_nums, len(all_user_ids))
...
def get_trn_val_tst_data(data_path, offline=True):
    if offline:
        ...
        click_trn, click_val, val_ans = trn_val_split(click_trn_data, VALID_USER_NUMS)
    else:
...
def trian_item_word2vec(click_df, embed_size=64, save_name='item_w2v_emb.pkl', split_char=' '):
    if Word2Vec is None:
        raise ModuleNotFoundError('gensim 未安装，无法训练 item_w2v_emb.pkl')
...
def get_embedding(save_path, all_click_df):
    if os.path.exists(save_path + 'item_content_emb.pkl'):
        ...
        raise FileNotFoundError(save_path + 'item_content_emb.pkl 文件不存在，请先运行 Recall.py 生成文章内容 embedding')
    ...
    if os.path.exists(save_path + 'item_w2v_emb.pkl'):
        ...
        print('item_w2v_emb.pkl 文件不存在，本脚本当前未使用该特征，跳过训练。')
...
click_trn, click_val, click_tst, val_ans = get_trn_val_tst_data(
    data_path,
    offline=OFFLINE,
)
...
def neg_sample_recall_data(recall_items_df, sample_rate=0.001):
    ...
    ratio = len(pos_data) / len(neg_data) if len(neg_data) else np.inf
    print('pos_data_num:', len(pos_data), 'neg_data_num:', len(neg_data), 'pos/neg:', ratio)
    if neg_data.empty:
        return pos_data
...
def get_user_recall_item_label_df(click_trn_hist, click_val_hist, click_tst_hist,click_trn_last, click_val_last, recall_list_df):
    ...
    if click_val_hist is not None:
        ...
        # Keep every recalled candidate in validation so ranking metrics are honest.
    else:
...
recall_list_dict = get_recall_list(save_path, single_recall_model=SINGLE_RECALL_MODEL, multi_recall=USE_MULTI_RECALL)
...
def label_df_to_tuples_dict(label_df):
    if label_df is None or label_df.empty:
        return {}

    tuple_series = label_df.groupby('user_id', group_keys=False)[['sim_item', 'score', 'label']].apply(
        lambda group_df: list(zip(group_df['sim_item'], group_df['score'], group_df['label']))
    )
    return tuple_series.to_dict()


trn_user_item_label_tuples_dict = label_df_to_tuples_dict(trn_user_item_label_df)

if val_user_item_label_df is not None:
    val_user_item_label_tuples_dict = label_df_to_tuples_dict(val_user_item_label_df)
else:
    val_user_item_label_tuples_dict = None
    
tst_user_item_label_tuples_dict = label_df_to_tuples_dict(tst_user_item_label_df)
...
def create_feature(users_id, recall_list, click_hist_df,  articles_info, articles_emb, user_emb=None, N=1):
    ...
    article_info_dict = articles_info.set_index('article_id')[['created_at_ts', 'words_count']].to_dict('index')
    hist_items_dict = click_hist_df.groupby('user_id')['click_article_id'].apply(
        lambda item_series: item_series.tail(N).tolist()
    ).to_dict()

    for user_id in tqdm(users_id):
        # 该用户的最后N次点击
        hist_user_items = hist_items_dict.get(user_id, [])
        if not hist_user_items:
            continue
        
        # 遍历该用户的召回列表
        for rank, (article_id, score, label) in enumerate(recall_list[user_id]):
            # 该文章建立时间, 字数
            article_info = article_info_dict.get(article_id)
            if article_info is None:
                continue
            a_create_time = article_info['created_at_ts']
            a_words_count = article_info['words_count']
            ...
            for hist_item in hist_user_items:
                hist_article_info = article_info_dict.get(hist_item)
                if hist_article_info is None:
                    continue
                b_create_time = hist_article_info['created_at_ts']
                b_words_count = hist_article_info['words_count']
                
                if hist_item not in articles_emb or article_id not in articles_emb:
                    continue
...
def active_level(all_data, cols):
    ...
    user_act = data.groupby('user_id', as_index=False).agg(
        click_size=('click_article_id', 'size'),
        click_timestamp=('click_timestamp', list),
    )
...
def hot_level(all_data, cols):
    ...
    article_hot = data.groupby('click_article_id', as_index=False).agg(
...
def user_cat_hob_fea(all_data, cols):
    ...
    :param cols: 用到的特征列
    """
    user_category_hob_info = all_data[cols].copy()
    user_category_hob_info = user_category_hob_info.groupby('user_id', as_index=False).agg(
        cate_list=('category_id', list)
    )
    
    user_cat_hob_info = pd.DataFrame()
    user_cat_hob_info['user_id'] = user_category_hob_info['user_id']
    user_cat_hob_info['cate_list'] = user_category_hob_info['cate_list']
...
def add_is_cat_hab(feature_df):
    if feature_df is None:
        return None
    if feature_df.empty:
        feature_df['is_cat_hab'] = pd.Series(dtype=np.int8)
        return feature_df
    feature_df['is_cat_hab'] = feature_df.apply(
        lambda x: 1 if x.category_id in set(x.cate_list) else 0,
        axis=1,
    )
    return feature_df

trn_user_item_feats_df = add_is_cat_hab(trn_user_item_feats_df)
val_user_item_feats_df = add_is_cat_hab(val_user_item_feats_df)
tst_user_item_feats_df = add_is_cat_hab(tst_user_item_feats_df)
